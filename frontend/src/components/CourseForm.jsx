...
const CourseForm = ({ onSubmit }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [trainerResponse, branchResponse, departmentResponse] = await Promise.all([
        ...
        if (departmentResponse.data.status) {
          setDepartmentOptions(departmentResponse.data.studentList);
        }
        if (departmentResponse.data.status) {
          setSchoolOptions(departmentResponse.data.schoolList);
        }
      } catch (error) {
...
useEffect(() => {
  ...
  const handleCourseCreation = () => {
    ...
    const token = localStorage.getItem('jwtToken');
    axios.get('http://localhost:8181/api/v1/course/create/', courseCreate,{
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      }
